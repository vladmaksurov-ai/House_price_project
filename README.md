...
Параметры моделей находятся в `src/house_price/config.py`. Параметры XGBoost подобраны
отдельным экспериментом с `RandomizedSearchCV`; остальные модели используют стартовые
конфигурации.
...
| XGBoost | **0.12666** | **0.01550** |

XGBoost с подобранными параметрами выбран текущей моделью по умолчанию.
